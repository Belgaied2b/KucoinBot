...
def is_bullish_divergence(df, indicator=None):
    if df is None or len(df) < 5:
        return False
    if indicator is None:
        indicator = compute_rsi(df['close'])
    extrema = []
    close = df['close']
    for i in range(2, len(df) - 2):
        if close[i] < close[i - 1] and close[i] < close[i + 1]:
            extrema.append((i, close[i], indicator.iloc[i]))
    if len(extrema) < 2:
        return False
    _, price1, ind1 = extrema[-2]
    _, price2, ind2 = extrema[-1]
    return price2 < price1 and ind2 > ind1


def is_bearish_divergence(df, indicator=None):
    if df is None or len(df) < 5:
        return False
    if indicator is None:
        indicator = compute_rsi(df['close'])
