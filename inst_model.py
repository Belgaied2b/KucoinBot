...
def FLOOR_BY_SYMBOL(symbol: str) -> float:
    s = symbol.upper()
...
class InstAutoTune:
    ...
    def thresholds(self, symbol: str) -> Tuple[float, float]:
        ...
        if not arr:
            return FLOOR_BY_SYMBOL(symbol), 0.0
        arr = sorted(arr)
        k = int(np.ceil(0.70 * len(arr)) - 1)
        k = max(0, min(len(arr)-1, k))
        req = max(arr[k], FLOOR_BY_SYMBOL(symbol))
...
def infer_side_from_cvd(cvd: float, cvd_base: float) -> str:
    return "long" if cvd >= cvd_base else "short"

def compute_institutional(symbol: str) -> Dict[str, Any]:
    fr   = funding_rates(symbol, limit=24)
    oi   = open_interest_hist(symbol, period="5m", limit=96)
    lsr  = long_short_ratio(symbol, period="5m", limit=96)
    kl   = klines(symbol, interval="5m", limit=240)
...
def decide_institutional(symbol: str, tuner: InstAutoTune) -> Dict[str, Any]:
    ...
    score_final = 1.0 + max(0.0, raw - req)
