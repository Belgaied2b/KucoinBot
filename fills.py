...
def _sign(ts, method, ep, body):
    ...
    sig = base64.b64encode(
        hmac.new(KUCOIN_API_SECRET.encode(), payload.encode(), hashlib.sha256).digest()
    )
    pph = base64.b64encode(
...
def wait_for_fill(order_id: str, timeout_s: int = 20, poll_ms: int = 300) -> dict:
    """
    Poll l'ordre jusqu'à timeout_s.
    On NE considère 'filled' que si dealSize > 0 (vrai fill).
    Si l'ordre est annulé/rejeté avec dealSize == 0, on remonte cancelled=True.
    """
    t0 = time.time()
    last = {}
    while time.time() - t0 < timeout_s:
        ...
        try:
            ...
            # Annulé / rejeté sans aucun fill => on sort proprement
