...
def analyze_signal(df, direction="long", btc_df=None, total_df=None):
    ...
    # Bougie confirmation + volume croissant
    ...
    volume_croissant = last_volume > prev_volume

    print(f"[{symbol}]   Bougie valide : {bougie_valide}")
    print(f"[{symbol}]   Volume ↑      : {volume_croissant}")

    if not bougie_valide or not volume_croissant:
        print(f"[{symbol}] ❌ Rejeté (bougie invalide ou volume non croissant)\n")
        return None
    ...
    # TP intelligent = RR >= 1.5 sur structure
