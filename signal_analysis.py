...
def analyze_signal(df, direction, btc_df, total_df, btc_d_df, symbol):
    try:
        ...
        pivot = close.iloc[-20]
        if direction == "long":
            ote_zone = (pivot * 0.786, pivot * 0.618)
        else:
            ote_zone = (pivot * 1.272, pivot * 1.618)
        in_ote = ote_zone[1] <= close.iloc[-1] <= ote_zone[0]
        ...
        for name, valid in checks.items():
            if name in ["OTE", "BOUGIE", "DIVERGENCE"]:
                if not valid:
        ...
        tp = entry + (entry - sl) * 2 if direction == "long" else entry - (sl - entry) * 2
        ...
        generate_chart(
            ...
            entry=entry, sl=sl, tp=tp, direction=direction.upper()
        )
