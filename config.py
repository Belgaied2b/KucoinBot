...
class Settings(BaseModel):
    ...
    oi_req_min: float        = Field(default_factory=lambda: _get_float("OI_REQ_MIN", 0.10))
    delta_req_min: float     = Field(default_factory=lambda: _get_float("DELTA_REQ_MIN", 0.10))
    funding_req_min: float   = Field(default_factory=lambda: _get_float("FUNDING_REQ_MIN", 0.05))
    liq_req_min: float       = Field(default_factory=lambda: _get_float("LIQ_REQ_MIN", 0.10))
    ...
    liq_notional_norm: float   = Field(default_factory=lambda: _get_float("LIQ_NOTIONAL_NORM", 50_000.0))
    ...
    persist_win: int          = Field(default_factory=lambda: _get_int("PERSIST_WIN", 1))
    persist_min_ok: int       = Field(default_factory=lambda: _get_int("PERSIST_MIN_OK", 1))
    symbol_cooldown_sec: int  = Field(default_factory=lambda: _get_int("SYMBOL_COOLDOWN_SEC", 60))
    ...
    max_maker_slippage_ticks: int = Field(default_factory=lambda: _get_int("MAX_MAKER_SLIPPAGE_TICKS", 10))
