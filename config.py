...
class Settings(BaseModel):
    ...
    # --- SYMBOLS (liste manuelle si AUTO_SYMBOLS=0) ---
    symbols: list[str] = Field(
        default_factory=lambda: [s.strip().upper() for s in os.getenv("SYMBOLS", "BTCUSDT,ETHUSDT,SOLUSDT").split(",") if s.strip()]
    )

    # --- DISCOVERY & SCAN ---
    auto_symbols: bool = Field(default_factory=lambda: os.getenv("AUTO_SYMBOLS", "1") in ("1", "true", "True"))
    ...
    breakeven_after_tp1: bool = Field(default_factory=lambda: os.getenv("MOVE_TO_BE_AFTER_TP1", "1") in ("1","true","True"))
    ...
    allow_tol_rr: bool   = Field(default_factory=lambda: os.getenv("ALLOW_TOLERANCE_RR", "1") in ("1","true","True"))
    ...
    # --- MACRO ---
    use_macro: bool = Field(default_factory=lambda: os.getenv("USE_MACRO", "1") in ("1","true","True"))
    use_total2: bool = Field(default_factory=lambda: os.getenv("USE_TOTAL2", "1") in ("1","true","True"))
    macro_refresh_minutes: int = Field(default_factory=lambda: int(os.getenv("MACRO_REFRESH_MINUTES", "5")))

    # --- EXECUTION TACTICS (V1.1) ---
    post_only_entries: bool = Field(default_factory=lambda: os.getenv("POST_ONLY_ENTRIES", "1") in ("1","true","True"))
    ...
    cancel_on_adverse: bool = Field(default_factory=lambda: os.getenv("CANCEL_ON_ADVERSE", "1") in ("1","true","True"))
    two_stage_entry: bool = Field(default_factory=lambda: os.getenv("TWO_STAGE_ENTRY", "1") in ("1","true","True"))
    stage1_fraction: float = Field(default_factory=lambda: float(os.getenv("STAGE1_FRACTION", "0.35")))

    # --- EXECUTION (V1.1-bis) ---
    use_ioc_fallback: bool = Field(default_factory=lambda: os.getenv("USE_IOC_FALLBACK", "1") in ("1","true","True"))
