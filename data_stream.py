...
class DataStream:
    """
    Gère les flux WebSocket OHLCV et orderbook pour plusieurs exchanges.
    """
    def __init__(self, exchanges: list, symbols: list, timeframe: str):
        # Instancie chaque client ccxt.pro
        self.exchanges   = {name: getattr(ccxtpro, name)() for name in exchanges}
        self.symbols     = symbols
        self.timeframe   = timeframe
        self.ohlcv       = defaultdict(dict)  # (exchange, symbol) -> dernière bougie
        self.orderbooks  = defaultdict(dict)  # (exchange, symbol) -> dernier carnet

    async def start(self, on_update):
        logger.info(f"[DataStream] Exchanges init : {list(self.exchanges.keys())}")
        # Charge les marchés pour que watch_ohlcv/watch_order_book fonctionne
        for name, ex in self.exchanges.items():
            await ex.load_markets()
            logger.info(f"[{name}] marchés chargés ({len(ex.symbols)} symboles)")

        logger.info(f"[DataStream] Démarrage watchers pour {self.symbols}")
        tasks = []
        for name, ex in self.exchanges.items():
    ...
    async def _ws_ohlcv(self, name, ex, symbol, on_update):
        """
        Récupère en continu des bougies, conserve & notifie uniquement la dernière.
        """
        while True:
            try:
                ...
                logger.info(f"[{name}][{symbol}][OHLCV] reçu : {latest}")
