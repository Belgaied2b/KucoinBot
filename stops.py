# stops.py — SL "desk pro": Liquidité (prioritaire) > Structure > ATR (dernier recours) + logs
...
def _detect_eq_local(df: pd.DataFrame, lookback: int, tol_rel: float) -> Tuple[List[float], List[float]]:
    ...
    # petit pattern 3-point (H==H==H / L==L==L) sur pas large
...
def _sl_long_from_liquidity(df: pd.DataFrame, entry: float, tick: float) -> Tuple[Optional[float], Optional[float], str]:
    ...
    eqh, eql = _detect_eq_local(df, lookback=int(LIQ_LOOKBACK), tol_rel=tol_rel)
...
def _sl_short_from_liquidity(df: pd.DataFrame, entry: float, tick: float) -> Tuple[Optional[float], Optional[float], str]:
...
def protective_stop_long(df: pd.DataFrame, entry: float, tick: float) -> float:
    """
    LONG — priorité stricte:
      1) Liquidité (equal lows) la plus proche sous l'entrée (+ buffers liq, adaptatifs)
      2) Structure: swing low
      3) ATR: entry - ATR_MULT_SL * ATR   (ultime recours)
      + Garde-fous (cap %, cap ATR, min ticks, alignement, bon côté)
    """
    atr_val = _compute_atr(df)

    sl_liq, lvl, src = _sl_long_from_liquidity(df, float(entry), float(tick))
    if sl_liq is not None:
        LOGGER.info("[SL] long via %s lvl=%.12f entry=%.12f -> raw=%.12f", src, lvl, entry, sl_liq)
        raw = float(sl_liq)
    else:
        ...
        if swing is not None:
            ...
            LOGGER.info("[SL] long via structure swing=%.12f entry=%.12f -> raw=%.12f", float(swing), entry, raw)
        else:
            sl_atr = float(entry) - float(ATR_MULT_SL) * float(atr_val)
            base = sl_atr * (1.0 - float(SL_BUFFER_PCT))
            raw = _round_to_tick(base, float(tick)) - float(SL_BUFFER_TICKS) * float(tick)
            LOGGER.info("[SL] long via atr_fallback atr=%.12f entry=%.12f -> raw=%.12f", float(atr_val), entry, raw)

    sl = _apply_common_clamps(entry=float(entry), sl_raw=float(raw), side="buy",
                              tick=float(tick), atr_value=float(atr_val))
    LOGGER.info("[SL] long final=%.12f (tick=%.12f)", sl, float(tick))
    return sl


def protective_stop_short(df: pd.DataFrame, entry: float, tick: float) -> float:
    """
    SHORT — priorité stricte:
      1) Liquidité (equal highs) la plus proche au-dessus de l'entrée (+ buffers liq, adaptatifs)
      2) Structure: swing high
      3) ATR: entry + ATR_MULT_SL * ATR   (ultime recours)
      + Garde-fous (cap %, cap ATR, min ticks, alignement, bon côté)
    """
    atr_val = _compute_atr(df)

    sl_liq, lvl, src = _sl_short_from_liquidity(df, float(entry), float(tick))
    if sl_liq is not None:
        LOGGER.info("[SL] short via %s lvl=%.12f entry=%.12f -> raw=%.12f", src, lvl, entry, sl_liq)
        raw = float(sl_liq)
    else:
        ...
        if swing is not None:
            base = float(swing) * (1.0 + float(SL_BUFFER_PCT))
            raw = _round_to_tick(base, float(tick)) + float(SL_BUFFER_TICKS) * float(tick)
            LOGGER.info("[SL] short via structure swing=%.12f entry=%.12f -> raw=%.12f", float(swing), entry, raw)
        else:
            sl_atr = float(entry) + float(ATR_MULT_SL) * float(atr_val)
            base = sl_atr * (1.0 + float(SL_BUFFER_PCT))
            raw = _round_to_tick(base, float(tick)) + float(SL_BUFFER_TICKS) * float(tick)
            LOGGER.info("[SL] short via atr_fallback atr=%.12f entry=%.12f -> raw=%.12f", float(atr_val), entry, raw)

    sl = _apply_common_clamps(entry=float(entry), sl_raw=float(raw), side="sell",
                              tick=float(tick), atr_value=float(atr_val))
    LOGGER.info("[SL] short final=%.12f (tick=%.12f)", sl, float(tick))
    return sl
